...
impl Engine {
    ...
    fn apply_due_dividends<A: Algorithm>(&mut self, algo: &mut A, tick_date: NaiveDate) {
        for (symbol, amount) in drain_due(&mut self.pending.dividends, tick_date) {
            if self.apply_dividend(&symbol, amount) {
                if self.ctx.log_config.corporate_events {
                    eprintln!("[event] {tick_date}: dividend {symbol}, {amount:.4}/share");
                }
    ...
    fn apply_split(&mut self, symbol: &str, ratio: f64, tick_time: DateTime<Utc>) {
        ...
        if let Some(p) = self.last_known_prices.get_mut(symbol) {
            *p /= ratio;
        }
        ...
        let price = self.last_known_prices.get(symbol).copied().unwrap_or(pos.avg_price);
    ...
    fn apply_dividend(&mut self, symbol: &str, amount: f64) -> bool {
    ...
    fn apply_financing(&mut self) {
        ...
        let price_of = |sym: &String, avg: f64| prices.get(sym).copied().unwrap_or(avg);
        ...
        if ctx.margin_interest_rate > 0.0 && ctx.portfolio.cash < 0.0 {
            ...
            let book = |longs_only: bool| -> Vec<(String, f64)> {
                ctx.portfolio
                    ...
                    .map(|p| {
                        (p.symbol.clone(), p.quantity.abs() * price_of(&p.symbol, p.avg_price))
                    })
                    .collect()
            };
    ...
    fn apply_rename(&mut self, old: &str, new: &str) {
        ...
        if let Some(hist) = self.ctx.history_store.remove(old) {
            self.ctx.history_store.entry(new.to_string()).or_insert(hist);
        }
        if let Some(price) = self.last_known_prices.remove(old) {
            self.last_known_prices.entry(new.to_string()).or_insert(price);
        }
        if let Some(day) = self.last_seen_day.remove(old) {
            let entry = self.last_seen_day.entry(new.to_string()).or_insert(day);
            *entry = (*entry).max(day);
        }
        if let Some(mut orders) = self.resting_book.remove(old) {
            for o in &mut orders {
                o.symbol = new.to_string();
            }
            self.resting_book.entry(new.to_string()).or_default().extend(orders);
        }
        ...
        if let Some(mut pos) = self.ctx.portfolio.positions.remove(old) {
            ...
            if merged.quantity.abs() > EPSILON {
                self.ctx.portfolio.positions.insert(new.to_string(), merged);
            }
        }
        ...
        if let Some(lt_old) = self.lifetimes.remove(old) {
            match self.lifetimes.get_mut(new) {
                None => {
                    self.lifetimes.insert(new.to_string(), lt_old);
                }
                Some(lt_new) => {
    ...
    fn close_delisted_positions<A: Algorithm>(
        ...
        let stale: Vec<String> = self
            ...
            .keys()
            .filter(|s| {
                let seen = self.last_seen_day.get(*s).copied().unwrap_or(self.day_index);
    ...
    fn force_close_delisted(&mut self, symbol: &str, tick_time: DateTime<Utc>) {
        ...
        // quote.
        let price = self.last_known_prices.get(symbol).copied().unwrap_or(avg)
            * (1.0 - self.ctx.delist_haircut);
