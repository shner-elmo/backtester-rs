...
impl Engine {
    ...
    fn feed_consolidators(&mut self, bars: &[(Symbol, Bar)]) {
        ...
        //
        // `benches/consolidators.rs` measures exactly this, one consolidator
        // per symbol over a 500-symbol universe: 334 ns of dispatch per bar
        // scanning, 20 ns indexed. Registries of one or two entries (every
        // example strategy) are unaffected either way.
