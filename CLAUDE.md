...
- `symbol.rs` — `Symbol` (a `Copy` u32 handle) and the `SymbolTable` that interns ticker strings, plus `SymbolMap` (hash map on symbols, multiply-hashed), `SymbolSet`, and `SymbolVec` (dense per-symbol array, used for the per-bar state: history, marks, last-seen day). Ticker strings are converted only at the edges — `Context::add_equity` on the way in, the result JSON on the way out — so nothing per-bar allocates or hashes text
...
- `data.rs` — Loads bars from Parquet files (arrow/parquet crates); maps encoded ticker IDs to symbols via JSON (`TickerResolver`: ticker id -> interned `Symbol` array, built once per run, which is also the subscription filter); Hive-partitioned files sorted by (year, month), rows time-sorted within each file; `TickReader` streams a file grouped into per-timestamp ticks (the engine's entry point), `read_bars_from_file` yields raw bars per file. Metadata filenames (ticker map, splits, dividends, renames) default to consts but are overridable per-run via `Context::set_*_file` setters (relative to the data root, or absolute)
