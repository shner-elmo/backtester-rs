...
- `docs/data-setup.md` — Parquet dataset layout (Hive `year=/month=` partitioning), column schema, the metadata JSON files (`encoded_tickers.json`, splits/dividends/renames) and their formats, `STONKS_DATA_ROOT`, SEC Form 4 insider transactions (`insider_transactions.json` + the `insider-fetch` downloader), regenerating the dataset from raw CSVs (`scripts/ingest_arrow.rs`), the committed test fixtures, helper examples
...
cargo run -p insider-fetch -- --start 2023q1 --end 2023q4 --out <data-root>/insider_transactions.json  # SEC Form 4 downloader (needs --user-agent or $SEC_USER_AGENT)
...
- `insider.rs` — Loads `insider_transactions.json` (SEC Form 4 open-market trades, produced by the `insider-fetch` crate) with the same streaming symbol-filtered pattern as the dividends loader. The engine never reads it — strategies load it in `initialize()` and act on `filing_date`
