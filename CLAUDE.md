...
- `data.rs` — Loads bars from Parquet files (arrow/parquet crates); `TickerMap` holds the dataset's id ↔ ticker naming (loaded before `initialize` so `add_equity` can return the dataset's id) and `SubscriptionMask` is the per-row filter the reader uses — a selective one (under half the dataset's ids) is pushed into the Parquet reader as a `RowFilter` on `ticker`, so OHLCV pages for unsubscribed rows are never decoded; Hive-partitioned files sorted by (year, month), rows time-sorted within each file; `TickReader` streams a file grouped into per-timestamp ticks (the engine's entry point), `read_bars_from_file` yields raw bars per file. Metadata filenames default to consts; splits/dividends/renames are overridable from `initialize` via `Context::set_*_file`, the ticker map via the `*_with_ticker_map` entry points (relative to the data root, or absolute)
