...
- `context.rs` — Execution context passed to algorithm: subscribe symbols, place market orders (`Market`, `SetHoldings`, `Liquidate`) and resting `limit_order`/`stop_order`s, access bar history (rolling 500-bar deque), register consolidators, set slippage/commission/margin/lot-size/financing/volume-participation models and toggles
